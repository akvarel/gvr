...
- strict evidence provider request with explicit source/snapshot classes, canonical `EvidenceSlotIdentity` opt-in for replaceable evidence, fingerprinted semantic coverage, separate integrity-protected `AuditObservation`, stable categorical issue and result contracts, fail-closed class compatibility, public capability-registry validation, and exact runtime registry contracts;
...
- optional atomic executor recording through an explicitly supplied durable store or unit of work, with exact multi-acquisition dependency tuples threaded through bundle, falsification, claim, and session records, session exact records derived from claim exact records and canonicalized by record fingerprint so multiple claims may use different exact records for one bundle domain, direct immutable dependencies, no raw-ID slot inference, no global storage discovery, and no returned `PASS` after persistence failure;
